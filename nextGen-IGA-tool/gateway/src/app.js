...
const RECOMMENDATION_URL = process.env.RECOMMENDATION_URL || "http://recommendation:3002";
...
app.post("/api/create/group", async (req, res) => {
  try {
    const { groupCn, owner, appName, riskLevel, riskScore } = req.body;
    if (!groupCn || !owner) {
    ...
    if (upstream.ok) {
      ...
      try {
        await db.query(
          `INSERT INTO applications (id, app_name, risk_level, risk_score, owner_id)
           VALUES (?, ?, ?, ?, ?)
           ON DUPLICATE KEY UPDATE 
          ...
          [groupCn, appName || groupCn, riskLevel || 'MEDIUM', riskScore || 0, owner]
        );
...
app.get("/api/recommendation/audit", async (req, res) => {
  try {
    const response = await fetch(`${RECOMMENDATION_URL}/api/recommendation/audit`, {
      headers: getIdentityHeaders(req)
    });
    const data = await response.json();
    res.status(response.status).json(data);
...
app.get("/api/recommendation/onboarding/:uid", async (req, res) => {
  try {
    ...
    const targetUrl = `${RECOMMENDATION_URL}/api/access-requests/onboarding/${uid}`;
...
app.get("/api/recommendation/team/:uid", async (req, res) => {
  try {
    const { uid } = req.params;
    const targetUrl = `${RECOMMENDATION_URL}/api/access-requests/team/${uid}`;
